...
- For goals that span several parts of a site (the account, a list, a payment), pass `plan`: milestones `[{do, done_when}]` in English, from what you know about the site (for example `{do: "open the unpaid ads", done_when: "the page lists my unpaid ads"}`). JEV aims at the current milestone and checks each `done_when` on the page; without a plan it finds the way itself (progress checks, menus, rollback).
- For forms (posting, booking, sign-up) give the goal and the details as params; the task finds the way in ("Post an ad", "Sell", the account menu), walks multi-step forms, skips optional steps and works through payment or plan dialogs that are part of the goal. Sign-in params that turn out not to be needed (already signed in) do not hold the task back. For long flows you can also split the work: one task to sign in, another to fill the form.
...
| `consult` (the task is stuck but keeps exploring safe steps) | Read the situation report (plan, what JEV sees, pages passed, what was tried, the page). Answer with `plan` (milestones that replace what is left), a `hint` (scope `"domain"` keeps site knowledge such as "paying for publication is the «Premium» button"), `goto` (url) or `pick`. If the task finds its way first, the question is withdrawn. |
