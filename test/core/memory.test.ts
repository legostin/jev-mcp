...
describe('site hints', () => {
  it('keep the step and param they were given for, and old rows still load', () => {
    ...
    db.exec("INSERT INTO site_hints VALUES ('h0', 'kolesa.kz', 'The body type filter (Кузов) is inside advanced search.', 1)");
    const mem = new MemoryStore(db);
    mem.addHint('kolesa.kz', 'Pick the brand from the chips', { step: 'fill_param', key: 'brand', about: 'car brand' });
    const hints = mem.hints('kolesa.kz');
